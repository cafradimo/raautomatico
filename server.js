...
app.get('/api/next-ra/:modelo/:matricula', async (req, res) => {
    ...
    try {
        // O MongoDB faz o incremento e retorna o valor atualizado atomaticamente (atomic operation)
        const result = await collection.findOneAndUpdate(
            { _id: chave },                // Filtra pela chave
            { $inc: { valor: 1 } },        // Incrementa em 1
            { upsert: true, returnDocument: 'after' } // Cria se não existir, e retorna o novo valor
        );

        // Extrai o valor atualizado (se for a primeira vez, result.value será { _id: chave, valor: 1 })
...
app.get('/api/contadores', async (req, res) => {
    ...
    try {
        ...
        // Converte para o formato antigo { chave: valor } para não quebrar seu front-end
        const contadores = {};
...
app.listen(PORT, () => {
    console.log(`🚀 Servidor rodando em http://localhost:${PORT}`);
>>>>>>> ea07b738c7842d7c838f05bdcce03f012483d047
});